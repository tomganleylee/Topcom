...
get_network_info() {
    ...
    if systemctl is-active --quiet hostapd; then
        info+="   WiFi AP: ✓ Active\n"
        # Read actual SSID from hostapd config
        local ssid=$(grep "^ssid=" /etc/hostapd/hostapd.conf 2>/dev/null | cut -d= -f2)
        local password=$(grep "^wpa_passphrase=" /etc/hostapd/hostapd.conf 2>/dev/null | cut -d= -f2)
        info+="   SSID: ${ssid:-Camera-Bridge}\n"
        info+="   Password: ${password:-camera123}\n"
    else
    ...
    # SMB Share info
    info+="📁 SMB Shares:\n"
    info+="   Photos: \\\\192.168.10.1\\photos\n"
    if [ -d "/srv/scanner/scans" ]; then
        info+="   Scanner: \\\\192.168.10.1\\scanner\n"
    fi
...
start_camera_wifi_ap() {
    ...
    if [ $status -eq 0 ]; then
        # Get actual SSID and password from config
        local ap_ssid=$(grep "^ssid=" /etc/hostapd/hostapd.conf 2>/dev/null | cut -d= -f2)
        local ap_pass=$(grep "^wpa_passphrase=" /etc/hostapd/hostapd.conf 2>/dev/null | cut -d= -f2)
        ...
        dialog --title "WiFi AP Active" --msgbox "Camera WiFi Access Point is now active!\n\nNetwork Information:\n━━━━━━━━━━━━━━━━━━━━━━━━\nSSID: ${ap_ssid:-Camera-Bridge}\nPassword: ${ap_pass:-camera123}\nIP Address: 192.168.10.1\n\nSMB Shares:\n  Photos: \\\\192.168.10.1\\photos$scanner_info\n  Username: camera\n  Password: camera123\n\nBoth WiFi and Ethernet cameras can now connect!" 22 70
    else
