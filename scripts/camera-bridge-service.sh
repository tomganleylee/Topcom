...
sync_to_dropbox() {
    ...
    # Get relative path from SMB share
    local rel_path="${file#$SMB_SHARE/}"
...
periodic_sync() {
    ...
    while true; do
        ...
        if check_internet && check_dropbox_config; then
            log_message "Starting periodic sync"
            ...
            # Full directory sync
            if sudo -u camerabridge rclone sync "$SMB_SHARE" "$DROPBOX_DEST" \
